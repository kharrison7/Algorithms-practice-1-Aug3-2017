    ...
    // YOUR CODE HERE
        ...
        for( let i = 0; i < length; i++ ){
            ...
            for( let j = 0; j < length; j++) {
                // This makes an array of 2 values that is nested in the pairs array.
                let combination = [str1, singles[ j ]];
...
function navigateRushHour( timeOfDay ){
    // YOUR CODE HERE
    let answer = "";
    // This asks if the time is  during rush hour. If so then go around.
    if( timeOfDay >= 600 && timeOfDay <= 900 || timeOfDay >= 1600 && timeOfDay <= 1800 ){
     answer = "The time of day is " + timeOfDay + ", I'm going around the city.";
    }
    // If not rush hour then go through.
    else{
     answer = "The time of day is " + timeOfDay + ", I'm going through the city.";
    }
    return answer;
}
...
let interpolate = function( start, end, count ){
    ...
    for( let i = 0; i < endPoint ; i++ ){
      let split_Point = start;
      split_Point += interval * i;
      list.push( split_Point );
    }
    return list;
}
...
// let interpolate = function( start, end, count ){
//   interpolated = [start];
//   let interval = (end - start)/countfor (let i=0; i < count; i++){
//     interpolated.push(start += interval)
//   }
//   return interpolate
